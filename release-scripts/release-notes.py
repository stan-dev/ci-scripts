...
parser = argparse.ArgumentParser(
    description="Get release notes from merged PRs since last release"
)
...
try:
    ...
    print(
        "No GITHUB_TOKEN environment variable found. This mail fail if you hit the rate limit."
    )
    header = {}

if args.time == "fetch":
    release_url = (
        "https://api.github.com/repos/stan-dev/" + repository + "/releases/latest"
    )
...
prs_url = (
    "https://api.github.com/repos/stan-dev/"
    + repository
    + "/pulls?state=closed&sort=created&per_page=100&sort=asc&page="
)
                ...
                if REPO_PR_FORMAT[repository] in body:
                    ...
                    for line in body.split("\r\n"):
                        if parsing_release_notes:
                            ...
                            line = line.strip()
                            if len(line) > 0:
                                if parsing_release_notes == 1:  # first line
                                    parsing_release_notes = (
                                        2  # next line would be second
                                    )
                                    print(" - " + line, end="")
                                else:
                                    print("\n\t" + line, end="")
